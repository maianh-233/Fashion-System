...
export default function PositionManagement() {
  ...
  const remove=async(item)=>{if(!await notification.confirm({title:"Xác nhận xóa vị trí",message:`Xóa vị trí ${item.name}? Vị trí đang được nhân viên sử dụng sẽ không thể xóa.`,confirmText:"Xóa vị trí",cancelText:"Hủy",destructive:true}))return;try{await positionApi.remove(item.id);await refresh("Đã xóa vị trí.");}catch(error){notification.error(positionErrorMessage(error));}};
